...
namespace ZEN.Application.Usecases.ProjectUC.Commands
{
    ...
    ) : ICommandHandler<DeleteProjectCommand, OkResponse>
    {
        public async Task<CTBaseResult<OkResponse>> Handle(DeleteProjectCommand request, CancellationToken cancellationToken)
        {
            ...
            // Xóa tất cả Tech liên quan đến project
            ...
            // Xóa tất cả UserProject liên quan đến project (có thể có nhiều user)
            var allUserProjects = await dbContext.UserProjects
                .Where(x => x.project_id == request.Project_Id)
                .ToListAsync(cancellationToken);
            foreach (var up in allUserProjects)
            {
                dbContext.UserProjects.Remove(up);
            }

            // Xóa project
            projectRepo.Remove(currentProject);

            // Lưu thay đổi - sửa điều kiện từ > 1 thành >= 0 (hoặc > 0)
            var savedChanges = await unitOfWork.SaveChangeAsync(cancellationToken);
            if (savedChanges > 0)
            {
