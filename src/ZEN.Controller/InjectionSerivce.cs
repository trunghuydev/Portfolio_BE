...
public static class InjectionService
{
        ...
        if (!RuntimeConfig.IsOnPremise)
        {
            ...
            if (!string.IsNullOrEmpty(connectionString))
            {
                services.AddHealthChecks()
                    .AddNpgSql(connectionString, name: "maindb");
            }
        }
    ...
    public static async Task ApplyWebBuilder(this WebApplication app)
    {
        ...
        // Endpoints that implement IEndpoint are automatically discovered and mapped via app.MapEndpoints(versionSet)
        // Endpoints that don't implement IEndpoint need to be mapped manually
        ...
        // PublicEndpoint implements IEndpoint, so it's already mapped automatically - don't map it again!
        // new PublicEndpoint().MapEndpoints(app, versionSet);
            ...
            app.UseSwaggerUI(options =>
            {
                ...
                foreach (var description in provider.ApiVersionDescriptions)
                {
                    options.SwaggerEndpoint(
                        ...
                        $"Portfolio API {description.GroupName.ToUpperInvariant()}");
                }
            });
