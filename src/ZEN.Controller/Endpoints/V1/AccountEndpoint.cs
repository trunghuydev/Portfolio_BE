...
public class AccountEndpointV1 : IEndpoint
{
    ...
    )
    {
        ...
        var normalizedUsername = ZEN.Domain.Common.Utils.UsernameValidator.Normalize(model.Username);
        if (!ZEN.Domain.Common.Utils.UsernameValidator.IsValid(normalizedUsername))
        {
            return Results.BadRequest(new { error = "INVALID_USERNAME", message = "Username must be 3-30 characters, only a-z, 0-9, -, _ allowed" });
        }

        // Check if username already exists (check both UserName and username field)
        var existingUser = await userManager.FindByNameAsync(model.Username);
        if (existingUser != null)
        {
            return Results.Conflict(new { error = "USERNAME_TAKEN", message = "Username already taken" });
        }

        // Check username field in database
        ...
        if (existingUser != null)
        {
            return Results.Conflict(new { error = "EMAIL_TAKEN", message = "Email already exists" });
        }
        ...
        var user = new AspUser
        {
            ...
            slug = ZEN.Domain.Common.Utils.UsernameValidator.GenerateSlug(normalizedUsername),
            is_public = true
        };
        ...
        {
            // Generate tokens
            var tokens = await tokenService.GenerateTokensAsync(user, context.GetIpv4Address(), false);

            return Results.Created("/api/v1/profile", new
            {
                user_id = user.Id,
        ...
        return Results.BadRequest(new { error = "REGISTRATION_FAILED", message = "Registration failed", errors = result.Errors });
    }
}
